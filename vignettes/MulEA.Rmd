...
If you would like to save the knowledge base as a GMT file, use: `MulEA::saveDataFrameAsGmtFile()` with two arguments `modelDF`, `gmtFilePath`.
...
```{r, results = 'asis', echo = TRUE} 
setBasedTest <- ORA(gmt = modelDfFromFile, testData = dataFromExperiment, nthreads = 2)
...
```{r, results = 'asis', echo = TRUE} 
setBasedTestWithPoolAndAdjust <- ORA(gmt = modelDfFromFile, testData = dataFromExperiment, pool = dataFromExperimentPool, adjustMethod = "PT", nthreads = 2)
...
## Ranking Based Tests
The ranked list based enrichment analysis needs an ordered list of genes (e.g. transcripts or proteins) as input. Ranking can be an ordered vector or any vector with a vector of scores, both of them having to be the same length. For now MulEA provides you two ranked based tests, the Kolmogorov-Smirnov test and the Subramanian test. Both of them are enclosed in the `RankedBasedTest` class, which provides you a method to set input data and configure other parameters including the used statistic methods.
...
Kolmogorov-Smirnov test is achieved by setting `method` argument to `"KS"`. It is also required to provide the argument for `testData`.
...
#rankedBasedTestKsRes <- MulEA::run_test(rankedBasedTestKs)
```
...
To use the Subramanian set the `method` argument to `"Subramanian"`, and input `testData` and `scores`. It is important that the latter two vectors have to be of the same length.
...
Below is the list of arguments accepted by the constructor of the `RankedBasedTest`:
...
- `testData` - required* - Vector of your experimental data. Example: `dataFromExperiment <- c("FBgn0004407", "FBgn0010438", "FBgn0003742")`. In case of KS test it ranks them. When applying the Subramanian method it creates a ranking using the scores argument.
- `scores` - if method="Subramanian": required*, default: numeric() - This argument is a vector of numbers. It creates a ranking with `testData` argument for Subramanian approach.
- `numberOfPermutations` - default: 1000 - This set number of permutations used to count p-value. You can speed up process of counting by set it to small value, but remember that it may impact the accuracy of your test.
