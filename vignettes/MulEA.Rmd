...
## Data Analysis
MulEA implements three types of tests, based on contingency tables, rankings and network topology.

### Set Based Tests
Set based tests are most commonly used in enrichment analysis tools. Counts of genes or other entities are collected in form of 2x2 contingency table with rows representing specific knowledge base category and genes in remaining categories and columns corresponding to set under study and remaining part of the background set. MulEA implements hypergeometric test as `SetBasedTest` class. This class is highly configurable and alows adjusting test's results by Benjamin-Hochberg and permutation tests.  

In order to start performing this tests, we have to prepare data in proper form. Commands below all steps neecessary to prepere input data.  
...
Basic usage of `SetBasedTest` class is presented as follows:
