...
To use the Subramanian set the `method` argument to `"Subramanian"`, and input `testData` and `element_scores`. It is important that the latter two vectors have to be of the same length.

```{r, results = 'asis', echo=TRUE} 
rankedBasedTestSubramanian <- RankedBasedTest(method = "Subramanian", gmt = modelDfFromFile, testData = dataFromExperiment, element_scores = dataFromExperimentScores)
...
- `testData` - required* - Vector of your experimental data. Example: `dataFromExperiment <- c("FBgn0004407", "FBgn0010438", "FBgn0003742")`. In case of KS test it ranks them. When applying the Subramanian method it creates a ranking using the element_scores argument.
- `element_scores` - if method="Subramanian": required*, default: numeric() - This argument is a vector of numbers. It creates a ranking with `testData` argument for Subramanian approach.
