...
```{r, results = 'asis', echo = TRUE}
setBasedTest <- ora(gmt = modelDfFromFile, element_names = dataFromExperiment, nthreads = 2)
...
```{r, results = 'asis', echo = TRUE}
setBasedTestWithPool <- ora(gmt = modelDfFromFile, element_names = dataFromExperiment, background_element_names = dataFromExperimentPool, nthreads = 2)
...
```{r, results = 'asis', echo = TRUE}
setBasedTestWithPoolAndAdjust <- ora(gmt = modelDfFromFile, element_names = dataFromExperiment, background_element_names = dataFromExperimentPool, p_value_adjustment_method = "BH", nthreads = 2)
setBasedTestWithPoolAndAdjustRes <- mulea::run_test(setBasedTestWithPoolAndAdjust)
```
...
-   `ontology_id` - input copy - Column copies from the input data frame. It include onlology ids. Could be for example ids from GO.
-   `ontology_name` - input copy - Column copies from the input data frame. It include onlology names. Could be for example name from GO as "mitochondrion inheritance".
-   `list_of_values` - input copy - Column copies from the input data frame. It include all symbols undet presented ontology id. Example from GO: FBgn0004407, FBgn0010438.
