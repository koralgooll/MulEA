    ...
    function(model = NULL,
        ...
        p_value_max_threshold = TRUE) {
            ...
            model_with_res <- merge(x = model@gmt, y = model_results,
                by.x = model_ontology_col_name, by.y = ontology_id_colname,
                all = TRUE)
...
plot_graph <- function(reshaped_results,
    ...
    p_value_max_threshold = 0.05) {
        ...
        graph_plot <- ggraph::ggraph(routes_tidy, 
            layout = "linear", circular = TRUE)
        if (0 != nrow(tibble::as_tibble(tidygraph::activate(
            routes_tidy, edges)))) {
...
plot_barplot <-
    function(reshaped_results,
        ...
        unique_reshaped_results <-
            unique(reshaped_results[selected_rows_to_plot, 
...
plot_heatmap <- function(reshaped_results,
    ...
    p_value_max_threshold = 0.05) {
        validate_column_names_and_function_args(data = reshaped_results,
            p_value_type_colname, ontology_element_colname, 
            ontology_id_colname)
        model_with_res_dt_relaxed <- filterRelaxedResultsForPlotting(
            reshaped_results = reshaped_results,
            p_value_type_colname = p_value_type_colname,
        ...
        ggplot(model_with_res_dt_relaxed_sort_pval, aes(
