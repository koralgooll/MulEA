...
plotGraph <- function(mulea_relaxed_resuts, edge_weight_cutoff=0, 
  ...
  for (i in 1:(nrow(ontologies)-1)) {
    ...
    genes_in_ontology_i <- model_with_res_dt_relaxed[ontologyId==ontology_name_i, ][[gen_id_in_ontology_column_name]]
    
    for (j in (i+1):nrow(ontologies)) {
      ontology_name_j <- ontologies[j, ontologyId]
      genes_in_ontology_j <- model_with_res_dt_relaxed[ontologyId==ontology_name_j, ][[gen_id_in_ontology_column_name]]
  ...
  nodes_p_stat <- model_with_res_dt_relaxed[[statistics_value_colname]]
...
plotHeatmap <- function(mulea_relaxed_resuts, 
  ...
  model_with_res_dt_relaxed_sort_pval <- model_with_res_dt_relaxed %>% 
    dplyr::arrange(., desc((!!rlang::sym(statistics_value_colname))), .by_group = FALSE)
  ...
  ggplot(model_with_res_dt_relaxed_sort_pval, 
         aes(!!rlang::sym(gen_id_in_ontology_column_name), ontologyId, fill= !!rlang::sym(statistics_value_colname))) + 
    scale_fill_gradient2(mid='darkgreen', high='red', limits=c(0.0, 1.0)) +
    geom_tile() +
    theme_light() +
