...
set.based.enrichment.test <- function(
  ...
  ## convert the database and the select and pollt to sorted integer lists
    ...
    # init empty data frame  with the row-size of DB (number of DB categories) 
    # to store the result
    ...
    for (i in seq_len(number_of_DB_categories))
    {
      ...
      result_df1$DB_in_select[i] <- length(
            intersect(select,current_DB_category))
      #m: number of common genes between DBterm and BackGround
      result_df1$DB_in_pool[i] <- length(intersect(pool,current_DB_category))
    ...
    if(nthread==1)
    {
        ...
        simulation_result_tbl <- tryCatch(
            enrichment_test_simulation(DB, list_of_all_genes, pool, 
                length(select), steps, 0), error = print)  # RCPP calling
    ...
    # 
    # simulation_result_tbl$intersect.size-1 ~ q vector of quantiles 
    # representing the number of white balls drawn without replacement from an 
    # urn which contains both black and white balls.
    #
    # simulation_result_tbl$DB_in_pool ~ m the number of white balls in the 
    # urn.
    # length(pool)-simulation_result_tbl$DB_in_pool ~ n the number of black 
    # balls in the urn.
    # length(select) ~ k the number of balls drawn from the urn, hence must 
    # be in 0,1,…, m+n.
    ...
    for(i in seq(list11))
    {
        ...
        {
            list11[[i]] <- data.frame(
                multiplicity = sum(list11[[i]]$multiplicity),
                p = list11[[i]]$p[1])
        }
    }
