...
set.based.enrichment.test <-
  ...
  for( i in seq(list11))
  {
    ...
    result_df1$P_val = 1 - phyper(
      result_df1$DB_in_select - 1,
      result_df1$DB_in_pool,
      size_pool - result_df1$DB_in_pool,
      size_select
    )
    ...
    } else if (nthread > 1 && round(nthread) == nthread)
    {
      ...
      result_of_paralel <-
        clusterApplyLB(cl = cl, 1:nthread, function(idx) {
          simulation_result_tbl <-  tryCatch(
            enrichment_test_simulation(
              DB,
              list_of_all_genes ,
    ...
    names(simulation_result_tbl) <-
      c("DB_in_pool", "intersect.size", "multiplicity") # set the column names
    simulation_result_tbl$p <-
      1 - phyper(
        simulation_result_tbl$intersect.size - 1,
        simulation_result_tbl$DB_in_pool,
        length(pool) - simulation_result_tbl$DB_in_pool,
        length(select)
      )
    ...
    if (!all(is.finite(simulation_result_tbl$p))) {
      stop("ERROR_002")
    } # TODO handle somehow the exception
    ...
    simulation_result_tbl <-
      simulation_result_tbl[, c("multiplicity", "p")]
    simulation_result_tbl$p <- round(simulation_result_tbl$p, digits = 15)
    simulation_result_tbl <-
      simulation_result_tbl[order(simulation_result_tbl$p), ]
    ...
    for (i in seq(list11))
    {
      ...
      {
        list11[[i]] <-
          data.frame(multiplicity = sum(list11[[i]]$multiplicity),
                     p = list11[[i]]$p[1])
      }
    }
    ...
    {
      simulation_result_tbl <-
        rbind(data.frame(multiplicity = 0, p = 0), simulation_result_tbl)
    }
    if (simulation_result_tbl$p[nrow(simulation_result_tbl)] != 1)
    {
      simulation_result_tbl <-
        rbind(simulation_result_tbl, data.frame(multiplicity = 0, p = 1))
    }
    
    simulation_result_tbl$cum_sum_multiplicity <-
      cumsum(simulation_result_tbl$multiplicity)
    ...
    NN <-
      simulation_result_tbl$cum_sum_multiplicity[nrow(simulation_result_tbl)] # this id the greatest/last in cumsum.multiplicity
