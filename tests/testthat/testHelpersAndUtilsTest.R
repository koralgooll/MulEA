...
test_that("Methods : checkIfPoolIncludeSample false", {
  ...
  testthat::expect_warning(MulEA:::checkIfPoolIncludeSample(
...
test_that("Methods : checkIfPoolIncludeSample false", {
  ...
  testthat::expect_equal(MulEA:::checkIfPoolIncludeSample(
    model = gmtMock, sampleVector = testDataMock), c("a", "b", "d"))
})
...
test_that("Methods : cutGmtToPool", {
  ...
  testthat::expect_equal(
    MulEA:::cutGmtToPool(gmt=gmtMock, pool=poolMock)[['listOfValues']][[1]], 
    list(c("a", "b"))[[1]])
})
